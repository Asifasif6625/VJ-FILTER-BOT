        ...
        if manual == False:
            ...
            try:
                if settings['auto_ffilter']:
                    ai_search = True
                    reply_msg = await message.reply_text(f"<b><i>searching Your Requsted Files 🙈</i></b>")
    ...
    if gl == False:
        ...
        if k == False:
            ...
            if files:
                ...
                ai_search = True
                reply_msg = await query.message.edit_text(f"<b><i>searching Your Requsted Files 🙈</i></b>")
        ...
        if len(message.text) < 100:
            ...
            if not files:
                if settings["spell_check"]:
                    ...
                    kum = return await reply_msg.edit_text(f"**•നിങ്ങൾ സേർച്ച് ചെയ്ത File Database ൽ ഇല്ല.**\n**•സ്പെല്ലിംഗ് ശെരി ആണോ എന്ന് പരിശോധിക്കുക.**\n\n**•No File Found For Your Query.**\n**•Make Sure Spelling Is Correct.**")
                    await sleep(10)
                    await kum.delete()
        else:
            return
